...
It is really an impressive finding. Given that the deep learning AI system AlphaTensor heavily relies on so many matrix operations, we could use it to improve the basic design of its own. Like what the company told, the trained algorithm would be plugged into the firmware of the CPU/GPU units which make the basic operations even faster.
HERE WE GO!
